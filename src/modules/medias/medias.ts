...
export default class Medias extends BaseModule {
  ...
  public async getAssetFiles(
    ...
    let params: IndexerModels.Asset['params'] = options.asset?.params;

    if (!params && this.stack.query) {
      const assetResponse = await this.stack.query.lookup.asset(id, {
        includeAll: true,
      });
      params = assetResponse?.asset?.params as
        | IndexerModels.Asset['params']
        | undefined;
    }
    ...
    // save cache
    if (this.cache) {
      await this.cache.save(CacheTable.MEDIAS_ASSET, files, { id, options });
    }
  ...
  public async getFiles(url: string) {
    ...
    if (media.type === MediaType.JSON) {
      ...
      const arc3Files = await this.getMedias(
        media.content as Record<string, any>,
      );
  ...
  private async getMedias(data: Record<string, any>) {
    ...
    // check for image
    if (data.image) {
      const media = await this.rateLimiter(() =>
        this.files.getMeta(data.image),
      );
      if (media.type === MediaType.IMAGE || media.type === MediaType.VIDEO) {
        mediaFiles.push(media);
      }
    } else if (data.external_url) {
      const image = await this.rateLimiter(() =>
        this.files.getMeta(data.external_url),
      );
    ...
    if (data.animation_url) {
      const animation = await this.rateLimiter(() =>
        this.files.getMeta(data.animation_url),
      );
    ...
    if (data.media_url) {
      const media = await this.rateLimiter(() =>
        this.files.getMeta(data.media_url),
      );
      if (media.type === MediaType.IMAGE || media.type === MediaType.VIDEO) {
        mediaFiles.push(media);
      }
    ...
    const arc69Txns = response.transactions
      ...
      .filter(
        (txn: Record<string, any>) =>
          ...
          String(txn.addons?.note.parsed?.json?.standard).toUpperCase() ===
            Arc.ARC69,
      )
  ...
  private mergeFiles(
    ...
    return {
      ...
      medias: [...baseFiles.medias, ...newFiles.medias],
    };
  }
